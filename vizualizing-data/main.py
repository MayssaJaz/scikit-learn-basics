...
def predict(dataset):
    nb = naive_bayes.MultinomialNB(fit_prior=True)# we choose the algorithem we are going to wrok with:Naive Bayes
    nb.fit(dataset.data[:-1], dataset.target[:-1]) # training stage: we are training our model on the whole dataset except the last element
    p31 = nb.predict([dataset.data[31]]) # predicting the class of the 32th instance
    print (p31)
    plast = nb.predict([dataset.data[-1]]) #predicting the class of the last instance
    print (plast)
    p = nb.predict(dataset.data[:]) # predicting the class of the whole dataset
    print (p)
    return (p)
...
def calculate_error(dataset,predictedDataset):
    ...
    if (choice ==2):
        ...
        print(f"Percentage of error (2nd method)--> {np.count_nonzero(myArray)/len(p_y)}")
